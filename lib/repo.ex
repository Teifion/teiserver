defmodule Teiserver.Repo do
  ...
  > #### Meant for Extending Teiserver {: .warning}
  >
  > These functions should only be used when working with a repo inside engines, plugins, or other
  > extensions for Teiserver. Favour using your application's repo directly when querying `Teiserver.Job`
  > from your workers.

  ## Examples

  The first argument for every function must be an `Teiserver.Config` struct. Many functions pass
  configuration around as a `conf` key, and it can always be fetched with `Teiserver.config/1`. This
  ...
  for {fun, arity} <- @callbacks_without_opts do
    ...
    @doc """
    Wraps `c:Ecto.Repo.#{fun}/#{arity}` with an additional `Teiserver.Config` argument.
    """
  ...
  @doc """
  The default values extracted from `Teiserver.Config` for use in all queries with options.
  """
  def default_options(conf) do
    base = [log: conf.log, Teiserver: true, telemetry_options: [teiserver_conf: conf]]
  ...
  @doc """
  Wraps `Ecto.Adapters.SQL.Repo.query/4` with an added `Teiserver.Config` argument.
  """
  ...
  @doc """
  Wraps `Ecto.Adapters.SQL.Repo.to_sql/2` with an added `Teiserver.Config` argument.
  """
